...
<div class="ofp-filters">
    <form method="GET" action="" class="ofp-filter-form">
        ...
        <?php if ( $filter_client ) : ?>
            <a href="<?php echo esc_url( admin_url( 'admin.php?page=ofp-billing' ) ); ?>" class="button">Clear</a>
        <?php endif; ?>
    </form>
</div>
    ...
    <?php else : ?>
        <div style="overflow-x:auto;">
            <table class="widefat ofp-table" style="min-width:1050px;">
                <thead><tr><th>Client</th><th>Plan</th><th>Amount (NGN)</th><th>Status</th><th>Period</th><th>Paid At</th><th>Actions</th></tr></thead>
                <tbody>
                <?php foreach ( $subscriptions as $sub ) : ?>
                    <tr>
                        ...
                        <td>
                            ...
                            $label = $sub->status === 'pending' ? 'Awaiting First Payment' : ucfirst( $sub->status );
                            echo '<span class="ofp-badge ' . esc_attr( $s_class ) . '">' . esc_html( $label ) . '</span>';
                        ...
                        <td>
                            <?php if ( $sub->status === 'pending' ) : ?>
                                <form method="POST" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline;">
                                    ...
                                    <button type="submit" class="button button-small button-primary" onclick="return confirm('Manually mark this CRM subscription as PAID?');">Mark Paid</button>
                                </form>
                            <?php else : ?>—<?php endif; ?>
                        </td>
                    </tr>
