...
class OFP_Property_Commerce {
    ...
    public static function create_purchase_from_offer( int $offer_id, string $accepted_ip = '' ) {
        ...
        if ( ! $offer ) return new WP_Error( 'offer_not_found', 'Installment offer was not found.' );
        if ( $offer->status !== 'pending' ) return new WP_Error( 'offer_not_pending', 'This offer is no longer available for acceptance.' );

        if ( $offer->expires_at && strtotime( $offer->expires_at ) < current_time( 'timestamp' ) ) {
            $wpdb->update( "{$p}ofp_property_offers", [ 'status' => 'expired', 'updated_at' => current_time( 'mysql' ) ], [ 'id' => $offer_id ] );
        ...
        try {
            $inserted = $wpdb->insert(
                ...
                [
                    ...
                    'payment_start_date' => $offer->payment_start_date ?? null,
            ...
            if ( ! $inserted ) throw new Exception( 'Unable to create purchase.' );
            ...
            if ( $initial > 0 ) {
                $wpdb->insert( "{$p}ofp_property_installments", [
            ...
            $wpdb->update( "{$p}ofp_property_offers", [ 'status' => 'accepted', 'accepted_at' => current_time( 'mysql' ), 'updated_at' => current_time( 'mysql' ) ], [ 'id' => $offer_id ] );
            $wpdb->query( 'COMMIT' );
            return $purchase_id;
        } catch ( Throwable $e ) {
            $wpdb->query( 'ROLLBACK' );
    ...
    public static function allocate_payment( int $payment_id ): array {
        ...
        $purchase = $wpdb->get_row( $wpdb->prepare( "SELECT purchase_type, total_price FROM {$p}ofp_property_purchases WHERE id = %d LIMIT 1", (int) $payment->purchase_id ) );

        if ( $purchase && $purchase->purchase_type === 'outright' ) {
            $already = (float) $wpdb->get_var( $wpdb->prepare( "SELECT COALESCE(SUM(amount),0) FROM {$p}ofp_property_payments WHERE purchase_id = %d AND status = 'successful' AND id <> %d", (int) $payment->purchase_id, $payment_id ) );
            $paid = $already + (float) $payment->amount;
            $balance = max( 0.0, (float) $purchase->total_price - $paid );
            $wpdb->update( "{$p}ofp_property_purchases", [ 'amount_paid' => $paid, 'balance' => $balance, 'status' => $balance <= 0.00001 ? 'completed' : 'active', 'updated_at' => current_time( 'mysql' ) ], [ 'id' => (int) $payment->purchase_id ] );
            return [ 'allocated' => (float) $payment->amount, 'unallocated' => max( 0.0, $paid - (float) $purchase->total_price ), 'installments_paid' => [] ];
        }

        $already = (float) $wpdb->get_var( $wpdb->prepare( "SELECT COALESCE(SUM(amount),0) FROM {$p}ofp_property_payment_allocations WHERE payment_id = %d", $payment_id ) );
        $remaining = max( 0.0, (float) $payment->amount - $already );
        $paid_ids = [];
        if ( $remaining <= 0 ) return [ 'allocated' => $already, 'unallocated' => 0.0, 'installments_paid' => [] ];

        $installments = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$p}ofp_property_installments WHERE purchase_id = %d AND status IN ('scheduled','due','partially_paid','overdue') AND amount_paid < amount_due ORDER BY installment_no ASC", (int) $payment->purchase_id ) );
        foreach ( $installments as $installment ) {
            ...
            $wpdb->insert( "{$p}ofp_property_payment_allocations", [ 'payment_id' => $payment_id, 'installment_id' => (int) $installment->id, 'amount' => $allocation, 'created_at' => current_time( 'mysql' ) ] );
            ...
            $wpdb->update( "{$p}ofp_property_installments", [ 'amount_paid' => $new_paid, 'status' => $new_status, 'paid_at' => $new_status === 'paid' ? current_time( 'mysql' ) : null, 'updated_at' => current_time( 'mysql' ) ], [ 'id' => (int) $installment->id ] );
            if ( $new_status === 'paid' ) $paid_ids[] = (int) $installment->id;
            $remaining -= $allocation;
        }

        $allocated = (float) $payment->amount - $remaining;
        $purchase_paid = (float) $wpdb->get_var( $wpdb->prepare( "SELECT COALESCE(SUM(amount),0) FROM {$p}ofp_property_payment_allocations a INNER JOIN {$p}ofp_property_payments pmt ON pmt.id = a.payment_id WHERE pmt.purchase_id = %d AND pmt.status = 'successful'", (int) $payment->purchase_id ) );
        $total_price = (float) $wpdb->get_var( $wpdb->prepare( "SELECT total_price FROM {$p}ofp_property_purchases WHERE id = %d", (int) $payment->purchase_id ) );
        $balance = max( 0.0, $total_price - $purchase_paid );
        $wpdb->update( "{$p}ofp_property_purchases", [ 'amount_paid' => $purchase_paid, 'balance' => $balance, 'status' => $balance <= 0.00001 ? 'completed' : 'active', 'updated_at' => current_time( 'mysql' ) ], [ 'id' => (int) $payment->purchase_id ] );

        return [ 'allocated' => $allocated, 'unallocated' => max( 0.0, $remaining ), 'installments_paid' => $paid_ids ];
    }
}
